 ...
 * @category schemas
 */
export const AnyToolCallPart: Schema.Codec<
  AnyToolCallPart,
  ToolCallPartEncoded
> = (() => {
 ...
 * @category guards
 */
export const isAnyToolResultPart = (
  u: unknown,
): u is RuntimeAnyToolResultPart =>
 ...
 * @category schemas
 */
export const AnyToolResultPart: Schema.Codec<
  AnyToolResultPart,
  ToolResultPartEncoded
> = (() => {
  const Common = {
...
const withAnyToolParts = (
  ...
  const unknownTool = <S extends Schema.Top>(part: S): S =>
    toolNames.size === 0 ? part : (part.pipe(
      Schema.check(
  ...
  return Schema.Union([
    schema,
    unknownTool(AnyToolCallPart),
    unknownTool(AnyToolResultPart),
  ]);
};
