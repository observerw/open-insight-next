...
import { FileSystem } from "#/Sandbox.ts";
...
export class Persist extends Context.Service<
  Persist,
  {
    ...
    ) => Effect.Effect<void, Trajectory.TrajectoryError, Tool.ResultEncodingServices<Tools[keyof Tools]>>;
    readonly load: (path: string) => Effect.Effect<Trajectory.Any, Trajectory.TrajectoryError>;
  }
    ...
    Effect.gen(function* () {
      ...
      const save = Effect.fn(function* <Tools extends Record<string, Tool.Any>>(
        ...
        const content = yield* Effect.try({
          ...
          catch: (cause) => Trajectory.persistenceError("save", path, cause),
        });

        yield* fs
          .writeFileString(path, content)
          .pipe(Effect.mapError((cause) => Trajectory.persistenceError("save", path, cause)));
      }) satisfies Persist["Service"]["save"];

      const load = Effect.fn(function* (path: string) {
        const content = yield* fs
          .readFileString(path)
          .pipe(Effect.mapError((cause) => Trajectory.persistenceError("load", path, cause)));

        const document = yield* Effect.try({ try: () => JSON.parse(content), catch: Trajectory.decodeError });

        const [metadata, parts] = yield* Effect.all([
          Schema.decodeUnknownEffect(Trajectory.Metadata)(document.metadata).pipe(
            Effect.mapError(Trajectory.decodeError),
          ),
