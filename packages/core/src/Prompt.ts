...
export type Provider<E = never> = Readonly<{
  _tag: "Provider";
  runSession(sandbox: Sandbox.Sandbox): Effect.Effect<Session<E>, E>;
}>;

type SessionOptions<E = never> = Readonly<{
  init: Prompt.RawInput;
  next: (response: Prompt.Prompt) => Effect.Effect<Prompt.RawInput | null, E, Sandbox.Sandbox>;
}>;

export type Options<E = never> = Readonly<{
  runSession(sandbox: Sandbox.Sandbox): Effect.Effect<SessionOptions<E>, E>;
}>;
