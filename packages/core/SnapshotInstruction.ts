...
export const user = (user: string): Instruction =>
  Instruction.make({ _tag: "User", user });
...
): Instruction => Instruction.make({ _tag: "Cmd", cmd: [program, ...args] });
...
export const copy = (
  ...
  dest: string,
  options: CopyOptions = {},
): Instruction => Instruction.make({ _tag: "Copy", src, dest, ...options });
