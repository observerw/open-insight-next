...
> = Readonly<{
  ...
  ): Stream.Stream<Response.StreamPartView<Tools>, HarnessError>;
}>;
...
const makeAgentSession = <Tools extends Record<string, Tool.Any>>(
  ...
  return {
    ...
    prompt: (prompt) =>
      agent.prompt(prompt).pipe(Stream.mapError(HarnessError.agent)),
...
export class Harness<ID extends string, Tools extends Record<string, Tool.Any>>
  extends Data.Class<{
    id: ID;
    metadata: Metadata;
...
export type ToolkitOf<H> = H extends Harness<any, infer Tools>
...
export const make = Effect.fn(
  function* <ID extends string, Tools extends Record<string, Tool.Any>>(
    id: ID,
    toolkit: Toolkit.Toolkit<Tools>,
    options: Options,
  ): Effect.fn.Return<
  ...
  > {
    ...
    const extendSnapshot =
      (template: Snapshot.Template) => (snapshot: Snapshot.Snapshot) =>
          ...
          Option.match({
            ...
            onSome: ({ instructions, context }) =>
              sandboxProvider
                ...
                .pipe(
                  Effect.mapError(HarnessError.snapshotDerive(instructions)),
                ),
          }),
        );

    const makeSandboxSession = Effect.fn("HarnessService.makeSandboxSession")(
      function* ({
        snapshot,
        options,
      }: Readonly<{
    ...
    const runSandbox = Effect.fn("HarnessService.runSandbox")(
