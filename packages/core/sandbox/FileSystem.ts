...
const childPath = (path: string, name: string): string =>
  `${directoryPrefix(path)}${name}`;
...
const make = Effect.fn(function* (
  ...
  const request = <A>(
    ...
    effect: Effect.Effect<
      A,
      WebDAV.OperationError,
      WebDAV.OperationRequirements
    >,
  ): Effect.Effect<A, SandboxError> =>
    effect.pipe(
      Effect.provideContext(context),
      Effect.mapError(operationFailed(operation, path)),
    );
  ...
  const put = (
    ...
    request(operation, path, client.putFileContents(path, data, { ...options }))
      .pipe(
        Effect.flatMap((written) =>
          written ? Effect.void : Effect.fail(
            operationFailed(operation, path)(
  ...
  const readStream = (
    ...
    return request(
      ...
      Effect.map(
        Stream.mapError((cause) => operationFailed(operation, path)(cause)),
      ),
      Stream.unwrap,
    );
  };
  ...
  const readBytes = (
    ...
    Stream.runCollect(readStream(operation, path)).pipe(
      Effect.map(concatChunks),
    );

  return FileSystem.of({
    access: (path) =>
      request("access", path, client.stat(path)).pipe(Effect.asVoid),

    copy: (fromPath, toPath, options) =>
      request(
        "copy",
        fromPath,
        client.copyFile(fromPath, toPath, { ...options, shallow: false }),
      ),
    ...
    glob: (pattern, options) => {
      ...
      return walk("glob", options?.root ?? "/").pipe(
        Effect.map((paths) =>
          paths.filter((path) => included(path) && !excluded(path))
        ),
      );
    },
    ...
    makeDirectory: (path, options) =>
      request(
        "makeDirectory",
        path,
        client.createDirectory(path, { ...options }),
      ),
    ...
    rename: (oldPath, newPath, options) =>
      request(
        "rename",
        oldPath,
        client.moveFile(oldPath, newPath, { ...options }),
      ),
    ...
    stat: (path) =>
      request("stat", path, client.stat(path)).pipe(Effect.map(toResourceInfo)),
