...
import { makeScript } from "./internal/utils.ts";
import { SandboxError } from "./SandboxError.ts";
...
type PlatformSpawn = (
  command: Command,
) => Effect.Effect<Result, PlatformError.PlatformError>;

const formatCommand = ({ command, args }: Command): string =>
  [command, ...args].join(" ");

export const make = (spawn: PlatformSpawn): Process["Service"] => {
  const spawnSandbox = (command: Command) =>
    spawn(command).pipe(
      Effect.mapError(SandboxError.process("spawn", formatCommand(command))),
    );

  const string: Process["Service"]["string"] = (command) =>
    spawnSandbox(command).pipe(
      Effect.map(({ stdout }) => new TextDecoder().decode(stdout)),
    );
  ...
  return Process.of({
    ...
    string,
    lines: (command) =>
      string(command).pipe(Effect.map((stdout) => stdout.split("\n"))),
    exitCode: (command) =>
      Effect.map(spawnSandbox(command), ({ exitCode }) => exitCode),
  });
};
